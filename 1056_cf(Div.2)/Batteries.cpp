...
void logic(){
        ...
        cin >> n;
        if (n == -1) exit(0);
        set<tuple<int, int, int>> querySet;

        floop(i,1,n+1){
            floop(j,i+1,n+1){
                int dist = j - i;
                querySet.insert({dist, i, j});
            }
        }
        priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<>> queryQueue;
        fauto(q,querySet){
            queryQueue.push(q);
        }
        while (!queryQueue.empty()) {
            auto [d, a, b] = queryQueue.top();
            queryQueue.pop();
            cout << a << sp << b << nl;
            cout.flush();
            int x;
