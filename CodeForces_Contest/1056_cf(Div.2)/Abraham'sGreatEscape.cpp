...
void logic(){
    ...
    if (m == 0) {
        ...
        queue<pint> columnPlan;
        unordered_set<int> usedCols;
        int col = 0;
        int remaining = m;

        while (remaining >= 2) {
            int height = min(n, remaining);
            columnPlan.push({col, height});
            usedCols.insert(col);
            col++;
            remaining -= height;
        }
        ...
        if (remaining > 0) {
            vector<pint> temp;
            while (!columnPlan.empty()) {
                temp.push_back(columnPlan.front());
                columnPlan.pop();
            }
            ...
            for(int i = temp.size() - 1; i >= 0; --i) {
                if (temp[i].second >= 3) {
                    temp[i].second--;
                    temp.push_back({col, 2});
                    usedCols.insert(col);
                    break;
                }
            }

            for (auto &p : temp) columnPlan.push(p);
        }

        while (!columnPlan.empty()) {
            auto [c, h] = columnPlan.front();
            columnPlan.pop();
            floop(r, 0, h - 1) {
                grid[r][c] = 'D';
            }
