...
void logic() {
    ...
    int lenOps; 
    cin >> lenOps;
    string insertChars, insertDirections;
    cin >> insertChars >> insertDirections;

    vector<char> resultVec(primaryStr.begin(), primaryStr.end());

    for (int i = 0; i < lenOps; ++i) {
        if (insertDirections[i] == 'V') {
            resultVec.insert(resultVec.begin(), insertChars[i]);
        } else {
            resultVec.push_back(insertChars[i]);
        }
    }

    string finalStr(resultVec.begin(), resultVec.end());
    cout << finalStr << endl;
}
int main(){
    int t;
